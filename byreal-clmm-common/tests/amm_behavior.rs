...
fn empty_amm(pool_key: Address, pool_state: PoolState) -> ByrealClmmAmm {
    ByrealClmmAmm {
        key: pool_key,
        pool_state,
...
fn tick_array_address_calculation() {
    let pool_key = Address::new_unique();
    let amm = empty_amm(pool_key, PoolState::default());

    let tick_array_addr = amm.get_tick_array_address(0);
    assert_ne!(tick_array_addr, Address::default());
    assert_ne!(
        amm.get_tick_array_address(100),
        amm.get_tick_array_address(200)
    );
}
...
fn decay_fee_disabled() {
    ...
    let amm = empty_amm(Address::new_unique(), pool_state);
...
fn compute_swap_errors_when_not_enough_tick_arrays() {
    ...
    let mut amm = empty_amm(Address::new_unique(), pool_state);
