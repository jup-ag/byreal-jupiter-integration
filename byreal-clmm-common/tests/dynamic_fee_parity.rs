...
fn arbitrage_fee_matches_contract_cases() {
    ...
    assert_eq!(calculate_arbitrage_fee(p_0, p_index, 0, 1_000).unwrap(), 9_000);
    assert_eq!(
    ...
    assert_eq!(
        calculate_arbitrage_fee(fixed_point_64::Q64 * 105 / 100, p_index, 5_000, 1_000)
            .unwrap(),
        44_000
    );
}
...
fn imbalance_fee_matches_contract_cases() {
    assert_eq!(calculate_imbalance_fee(150, 50, 5, 10, false).unwrap(), 200_000);
    assert_eq!(calculate_imbalance_fee(150, 50, 5, 10, true).unwrap(), 0);
    assert_eq!(calculate_imbalance_fee(100, 100, 5, 10, false).unwrap(), 0);
    assert_eq!(calculate_imbalance_fee(50, 150, 5, 10, true).unwrap(), 200_000);
