...
impl DynamicTickArrayState {
    ...
    /// Find the next initialized tick in this array.
    /// For dynamic arrays, delegates to on-chain helper methods that consult both
    /// the bitmap and TickState::is_initialized(), so that we don't stop on merely
    /// allocated but uninitialized ticks.
